...
export default function Page(Props: props) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => {
      return axios.get(
        `${process.env.NEXT_PUBLIC_DOMAIN}/backend-api/department/info-department?code=${params.id}`,
        {
  ...
  const editMutation = useMutation({
    ...
    mutationFn: (code1: string) => {
      return axios.put(
        `${process.env.NEXT_PUBLIC_DOMAIN}/backend-api/department/edit-department?code=${code1}`,
        { name, code },
        {
          withCredentials: true,
        }
